    ...
    IStringLocalizer<ExpensesService> loc) : IExpensesService
{
        ...
        CreateExpenseRequestDto request)
    {
        ...
        // Delegate validation + entity creation to the shared Core service (same
        // path used by background jobs). Pass through the Result unchanged.
        var creationResult = await expenseCreation.CreateValidatedExpenseAsync(
            ...
            request.Splits?.Select(s => new ExpenseSplitInput(s.UserId, s.SplitAmount)).ToList(),
            request.AliasSplits?.Select(s => new ExpenseAliasSplitInput(s.AliasId, s.SplitAmount)).ToList());
        ...
        if (group.UseAliases)
        {
            // Alias splits were built with Alias nav populated by the creation service
            var aliasSplitsWithAlias = expense.ExpenseAliasSplits.ToList();
        ...
        {
            // Splits were built with User nav populated by the creation service
            var splitsWithUsers = expense.ExpenseSplits.ToList();
