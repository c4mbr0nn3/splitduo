...
public class Result<T>
{
    ...
    public static Result<T> Unauthorized(string error = "Unauthorized access") => new(error, HttpStatusCode.Unauthorized);
    ...
    public static Result<T> UnprocessableEntity(string error = "Unprocessable entity") => new(error, HttpStatusCode.UnprocessableEntity);
    public static Result<T> InternalServerError(string error = "Internal server error") => new(error, HttpStatusCode.InternalServerError);
}
...
public class Result
{
    ...
    private Result(bool isSuccess, string error = "", HttpStatusCode statusCode = HttpStatusCode.OK)
    {
        IsSuccess = isSuccess;
        Error = error;
        StatusCode = isSuccess ? statusCode : (statusCode == HttpStatusCode.OK ? HttpStatusCode.BadRequest : statusCode);
    }

    public static Result Success(HttpStatusCode statusCode = HttpStatusCode.OK) => new(true, "", statusCode);
    public static Result Failure(string error, HttpStatusCode statusCode = HttpStatusCode.BadRequest) => new(false, error, statusCode);
    
    // Common failure shortcuts
    public static Result NotFound(string error = "Resource not found") => new(false, error, HttpStatusCode.NotFound);
    public static Result Unauthorized(string error = "Unauthorized access") => new(false, error, HttpStatusCode.Unauthorized);
    ...
    public static Result UnprocessableEntity(string error = "Unprocessable entity") => new(false, error, HttpStatusCode.UnprocessableEntity);
    public static Result InternalServerError(string error = "Internal server error") => new(false, error, HttpStatusCode.InternalServerError);
}