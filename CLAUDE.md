...
- CI lives in `ci/*.yml` (one file per concern) included from `.gitlab-ci.yml` — add new CI concerns as a new file under `ci/` and include it there.
- CI scanner/third-party images are pinned by digest (`image@sha256:...`) for supply-chain integrity — bump the digest deliberately after verifying the upstream image, never use a bare mutable tag.
