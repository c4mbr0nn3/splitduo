      ...
      <template v-else-if="stats">
        <div class="space-y-6">
          ...
          <div>
            ...
            <div class="space-y-3">
              <UCard
                v-for="balance in stats.balances"
                :key="balance.userId"
                variant="outline"
              >
...
const { groupStats, fetchGroupStats, isLoading } = useBalances(groupId)

const group = computed(() => currentGroup.value)
const stats = computed(() => groupStats.value)
...
useHead({
  title: computed(() => group.value ? `${group.value.name} — Stats` : 'Stats'),
})
