...
- **Settlements** — confirm flow is a page (`settle/confirm`, query-param prefill), not a modal. Expense cards branch on `expense.expenseTypeId === 1`: compute the display title frontend-side from `paidByUser` + split/alias-split data (no backend field), badge it, and suppress the edit affordance (delete only).