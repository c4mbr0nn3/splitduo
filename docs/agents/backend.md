...
- **Settlements are typed expenses** — `ExpenseTypeId == Settlement` + category `Settlement` (12), created only via `SettlementsService`; normal expense create/update rejects the category, `GET /categories` hides it. Excluded from group spending stats only (`GetGroupStatsAsync`); included in balances, net-position, and CSV export (importer round-trips the type flag).